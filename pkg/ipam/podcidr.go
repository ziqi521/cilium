...
func (s *nodeCIDRs) String() string {
	...
	for _, c := range s.v4PodCIDRs {
		cidrs = append(cidrs, c.String())
	}
	for _, c := range s.v6PodCIDRs {
		cidrs = append(cidrs, c.String())
	}
...
func syncToK8s(nodeGetter CiliumNodeGetterUpdater, ciliumNodesToK8s map[string]*ciliumNodeK8sOp) (retErr error) {
	for nodeName, nodeToK8s := range ciliumNodesToK8s {
		...
		case k8sOpUpdate:
			...
			updatedNode, err = nodeGetter.Update(nil, nodeToK8s.ciliumNode)
		...
		case k8sErrors.IsAlreadyExists(err) || k8sErrors.IsConflict(err):
			retErr = err
			newCiliumNode, err2 = nodeGetter.Get(nodeToK8s.ciliumNode.GetName())
			if err2 == nil {
	...
	if len(n.v6CIDRAllocators) != 0 {
		for _, ipNet := range cidrs.v6PodCIDRs {
			for _, v6ClusterCIDR := range n.v6CIDRAllocators {
				if v6ClusterCIDR.InRange(ipNet) {
					err := v6ClusterCIDR.Release(ipNet)
					log = log.WithFields(logrus.Fields{
						"cidr": ipNet.String(),
					})
					if err != nil {
	...
	for _, newCIDR := range newCidrs {
		...
		for _, cidrSet := range cidrSets {
			...
			if !cidrSet.InRange(newCIDR) {
				err = fmt.Errorf("Cilium Operator not configured for the requested CIDR %s", newCIDR)
				continue
			}
			if cidrSet.IsFull() {
