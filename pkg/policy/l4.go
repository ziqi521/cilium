...
func (npm NamedPortsMap) GetNamedPort(name string, proto uint8) (port uint16, err error) {
	...
	np, ok := npm[name]
	if !ok {
		return 0, fmt.Errorf("named port not found (%v)", npm)
	}
		...
		if port == 0 && l4Filter.PortName != "" {
			...
			if err != nil {
				if option.Config.Debug {
					...
					logger.Debugf("AccumulateMapChanges: Skipping named port: %s", err)
				}
				continue
			}
		}
