...
var (
	...
	wantL3n4AddrID = &FrontendID{
		ID:       123,
		L3n4Addr: l3n4Addr2,
	}
)

func (s *LBSuite) TestServices(c *C) {
	var nilL3n4AddrID *FrontendID
	...
	id, err := acquireGlobalID(l3n4Addr1, 0)
	c.Assert(err, Equals, nil)
	c.Assert(id, Equals, ffsIDu16)

	id, err = acquireGlobalID(l3n4Addr1, 0)
	c.Assert(err, Equals, nil)
	c.Assert(id, Equals, ffsIDu16)

	id, err = acquireGlobalID(l3n4Addr2, 0)
	c.Assert(err, Equals, nil)
	c.Assert(id, Equals, ffsIDu16+1)

	// l3n4Addr3 should have the same ID as l3n4Addr2 since we are omitting the
	// protocol type.
	id, err = acquireGlobalID(l3n4Addr3, 0)
	c.Assert(err, Equals, nil)
	c.Assert(id, Equals, ffsIDu16+1)
	...
	id, err = acquireGlobalID(l3n4Addr2, 0)
	c.Assert(err, Equals, nil)
	c.Assert(id, Equals, types.ServiceID(common.FirstFreeServiceID+1))
	...
	id, err = acquireGlobalID(l3n4Addr1, 99)
	c.Assert(err, Equals, nil)
	c.Assert(id, Equals, types.ServiceID(common.FirstFreeServiceID+1))
	...
	id, err = acquireGlobalID(l3n4Addr1, 99)
	c.Assert(err, Equals, nil)
	c.Assert(id, Equals, types.ServiceID(99))
}

func (s *LBSuite) TestGetMaxServiceID(c *C) {
