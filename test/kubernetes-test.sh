...
${HOME}/go/bin/kubetest --test --test_args="--ginkgo.focus=NetworkPolicy.*on.one.named.port --e2e-verify-service-account=false --host ${KUBE_MASTER_URL} --ginkgo.skip=(should.allow.egress.access.to.server.in.CIDR.block)|(should.allow.ingress.access.from.updated.pod)"
