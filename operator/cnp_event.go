		...
		case event, ok := <-watcher.Events:
			...
			case kvstore.EventTypeCreate, kvstore.EventTypeModify:
				...
				log.WithFields(logrus.Fields{
					...
					"key":       event.Key,
					"type":      event.Typ,
				}).Info("received event from kvstore")
					...
					controllers.UpdateController(nameNamespace, controller.ControllerParams{
						...
						Loop:
							...
							if len(nodeStatusMap) == 0 {
								//log.Infof("no updates to perform for %s, returning", nameNamespace)
								return nil
							}
							...
							if err := k8s.UpdateStatusesByCapabilities(k8sversion.Capabilities(), namespace, name, nodeStatusMap, k8s.CiliumClient(), nil); err != nil {
								return err
							}

							log.Infof("UpdateStatusesByCapabilities: node status updates to perform: %d", len(nodeStatusMap))
